...
class PartDataset(data.Dataset):
    def __init__(self, root, npoints=2500, pic2point=True, class_choice=None, train=True):
        ...
        for item in self.cat:
            ...
            dir_pic = os.path.join(self.root, self.cat[item], 'seg_img')
    ...
    def __getitem__(self, index):
        ...
        #print(point_set.shape, seg.shape, im.shape)

        # resample
        choice = np.random.choice(len(seg), self.npoints, replace=True)  # random choice n id for selection
